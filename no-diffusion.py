# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 12:07:14 2020
...
K = 20; # number of grid points (#20)
kpar = 20; 
#GP = 62.5
GP = 5;  #max growth rate (1 phyto per day) ; 0.001 - 10 "realistic range"
GS = 1.5;
...
dt = 0.01 # time step
nt = 10000 # number of time steps = 10,000
time = np.arange(nt+1)*dt   #won't work bc it saves each time
t_val = np.ones(z.shape)
tsave = [t_val]
...
#time-stepping diffusion
    #N = diffusion_time_stepping(dz,dt,N0,N) 
    #PE = diffusion_time_stepping(dz,dt,0,PE)
    #PS = diffusion_time_stepping(dz,dt,0,PS)

    if np.mod(t,10) == 0:   #save every 10 time steps 
        ...
        tsave = np.concatenate((tsave,[t_val]),axis=0)
